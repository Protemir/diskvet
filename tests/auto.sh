...
if sh "$ROOT/diskvet.sh" report --docker auto >"$OUT/auto-report.md" 2>"$OUT/auto-report.err"; then ok "report with --docker auto from the compose folder"; else fail "report: $(cat "$OUT/auto-report.err")"; fi
...
x=$(docker exec "$id" clickhouse-client --user clickhouse --password clickhouse -q "SELECT arrayStringConcat(groupUniqArray(user), ',') FROM system.query_log WHERE log_comment = 'diskvet' AND type = 'QueryFinish'" 2>&1)
if [ "$x" = clickhouse ]; then ok "queries ran as the container's CLICKHOUSE_USER, tagged log_comment=diskvet"; else fail "users seen in query_log: '$x'"; fi
x=$(docker exec "$id" clickhouse-client --user clickhouse --password clickhouse -q "SELECT countIf(query_kind != 'Select') FROM system.query_log WHERE log_comment = 'diskvet' AND type = 'QueryFinish'" 2>&1)
...
sh "$ROOT/diskvet.sh" --print-payload --docker auto --env "$ROOT/tests/fixtures/test.env" >"$OUT/auto-payload.json" 2>"$OUT/auto-payload.err"
...
if [ "$(docker ps --format '{{.Image}}' | tr -d '\r' | grep -c 'clickhouse-server')" = 0 ]; then
    sh diskvet.sh report --docker auto >/dev/null 2>"$OUT/auto-none.err"
    rc=$?
