...
|---|---|---|---|
| `tests/k8s.sh` (the `kubernetes` CI job) | kind, Kubernetes 1.37 and kubectl 1.37, with the API server's audit log on: the ClickHouse operator 0.0.7 with the ClickHouseCluster of the Langfuse chart 2.1.2 (ClickHouse 26.4), the Bitnami ClickHouse chart 8.0.5 on its own with 2 replicas (25.2), and a plain StatefulSet of `clickhouse/clickhouse-server:25.12` with a sidecar listed first; then the Bitnami chart 9.4.4 on its own (25.7) and the Altinity operator 0.27.4 with a ClickHouseInstallation of its own (25.12, a sidecar listed first) | dash + mawk | `--k8s auto` across namespaces (exactly the 4 server pods of the first three installs, no Keeper, operator, cert-manager or version-probe pod; with `--print-payload` one `ch_unreachable` line); a report per install, also `-n NS --k8s auto` past the sidecar and one with `--context`: all 7 checks run, the pod's own login in the header, the Fix B of its chart; payloads without cluster names, with the names hashed in the pod; ServiceAccounts with the Role of [Permissions](#permissions) (a named pod, and `--k8s auto` with its fallback note), with `get` only, and without `pods/exec` (exit 3 with the hint); no password or salt in the audit log, and no TTY in diskvet's exec lines there; unchanged pods (UID, spec, no ephemeral containers, restart counts) over every diskvet run. For the operator 0.0.7, Bitnami 8.x and plain installs also: only read-only SELECTs in `query_log`, no password or salt in `query_log` or `text_log`, and the flag line and Fix A through the printed clickhouse-client command; on the operator's pod the server log files on the data volume and the printed `rm` line (on a dummy rotated file). Then Fix B through the chart, with the TTLs checked after the restart: `helm template` for the operator (and the logger at level information), `helm upgrade --reuse-values` for Bitnami 8.x and 9.x, the printed block merged into the ClickHouseInstallation for Altinity; then Fix C as printed (not for Bitnami 9.x). For Bitnami 9.x the login from the password file and a `00-` file loading before the chart's `08-sampling.xml`; for Altinity the passwordless login, and that the operator does not restart the pod for a changed file: the report's `ls` line shows the file in the pod, and its `kubectl delete pod` line restarts it |
